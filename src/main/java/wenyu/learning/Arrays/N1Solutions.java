 ...
 * 			   2. Scan one by one or binary search [O(nlogn)/O(1)]
 * Solution 2: using extra space to mark exist number [O(n)/O(n)]
 * Solution 3: rearrange array, put number back to its index [O(n)/O(1)]
 */
