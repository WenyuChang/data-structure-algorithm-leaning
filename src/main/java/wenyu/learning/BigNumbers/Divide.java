...
public class Divide {
	...
	private static char[] divide(char[] number1, char[] number2, boolean minus) {
		...
		for(int i=0;i<number1.length;i++) {
			...
			char[] tmpResult = Multiple.multiplyInterface(number2.clone(), tmp);
			...
			while(tmpResult.length>0&&tmpResult[0]!='-') {
				...
				tmpResult = Multiple.multiplyInterface(number2.clone(), tmp);
			...
			tmpNum1 = Multiple.multiplyInterface(tmpNum1, "10".toCharArray());
		}
