...
/*
 * Combination:
 * 0. selectCount (Binomial Coefficient)  ==> C(n, m) = n!/((n-m)!*m!) 
 * 1. selectRecursion
 * 2. selectNoRecursion
...
public class Combination {
	...
	public static long selectCount(int N, int M, boolean print) {
		/*
		 * Logic:
		 * C(n, k) = n! / (n-k)! * k!
	    ...
	    for (int i=0; i<m; i++) {
	    	// Calculate value of [n * (n-1) *---* (n-M+1)] / [M * (M-1) *----* 1]
	    	result *= (n - i);
	    	result /= (i + 1);
	    }
		if(print) System.out.println("C(" + N + ", " + M + ") = " + result);
		return result;
	}
	...
	public static ArrayList<String> selectBasedOnBitwise(char[] list, int k, boolean print) {
		...
		while(result >= 0) {
			...
			arrayList.add(builder.toString());
			System.out.println();
			result = NextBigProblems.find(bitArr);
		}
	...
	public static void main(String[] args) {
		...
		selectCount(list.length, 2, true);
		
		System.out.println("Non-recursion:");
		//Select without recursion
		selectNoRecursion(list, 2, true);
//		
