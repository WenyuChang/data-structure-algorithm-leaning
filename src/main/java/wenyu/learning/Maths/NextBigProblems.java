...
/*
 * Problem 1: Rearrange the order of digits of given number, to find the next bigger number.
 * logic: O(n)
...
public class NextBigProblems {
	...
	public static void find(int[] numStr) {
		for(int i=numStr.length-2; i>=0; i--) {
			if(numStr[i] < numStr[i+1]) {
				// Find first digit greater than current digit
				for(int j=numStr.length-1; j>i; j--) {
				...
				System.out.println("Next bigger number is " + Arrays.toString(numStr));
				return;
			}
		}
		System.out.println("No such number...");
